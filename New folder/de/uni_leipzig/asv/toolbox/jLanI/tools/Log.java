...
/**
 * @author Michael Welt
 * Simple Logger, which supports Debugmode, and 3 cases
 * of Logmode.<br>
 * case 1: only Stdin / Stderr
 * case 2: only file logging (timestamp.log)
 * case 3: case 1 and 2 together
 * case 0: no log at all.
 * default: case 3
 * furthermore it supports bufferLimit, if the
 * given limit exeeds the buffer is saved automatically.
 *
 */
public class Log {
	...
	/**
	 * Is saved to a File timeStamp_count.log if count
	 * is zero nothing is appended. Count counts the saveOps.
	 *
	 */
	public void saveLog()
	{
		try{
			File logFile = new File(this.logPrefix+this.initTime+(count++>0?"_"+count:"")+".log");
	...
	{
		if(this.bufferLimit != -1 && this.logBuffer.toString().length()+string.length() > this.bufferLimit) {
			saveLog();
		}
	...
	{
		String logString = (this.timeStampMode?"(" + getTime() + ")":"") +" LOG : " + string+"\n";
		write(logString,false);
	}
	
	public synchronized void err(String string)
	{
		String logString = (this.timeStampMode?"(" + getTime() + ")":"") + " ERR : " + string+"\n";
	...
	public synchronized void debug(String string)
	{
		if(this.debugMode)
		{
			String logString = (this.timeStampMode?"(" + getTime() + ")":"")+" DEBUG : " + string+"\n";
			write(logString,false);
		}		
	}
