...
public class LangManager 
{
	...
	public static int addNewLanguage(String lang, String file) throws DataSourceException
	{
		...
		kern.dumpDatasource(lang, wordlistDir + System.getProperty("file.separator") + lang + ".ser.gz");
	...
	{
		return (kern.datasourcemanager.hasLanguage(lang)) ?
			   ((DataSource)kern.datasourcemanager.datasources.get(lang)).size() :
	...
	public static int addNewLanguageFromDb(String lang, HashMap wordlist) throws DataSourceException
	{
		...
		kern.datasourcemanager.datasources.put(lang, new DataSource(lang, wordlist));
		kern.dumpDatasource(lang, wordlistDir + System.getProperty("file.separator") + lang + ".ser.gz");
		if(inactiv.containsKey(lang)) {
			activ(lang, false);
		}
	...
	public static HashMap getAllLanguages()
	{
		...
		try
		{
			...
			Iterator it = kern.datasourcemanager.datasources.values().iterator();
			while (it.hasNext())
			{
	...
	public static void delete(String lang)
	{
		try
		{
			//LanIKernel kern = LanIKernel.getInstance();
			String wordlistDir = kern.prefs.getProperty( "WordlistDir" );
			File delFile1 = new File(wordlistDir + System.getProperty("file.separator") + lang + ".ser.gz");
			File delFile2 = new File(wordlistDir + System.getProperty("file.separator") + lang + ".txt");
			delFile1.delete();
			delFile2.delete();
	...
	public static boolean containsLanguage(String lang)
	{
		...
		retval= kern.datasourcemanager.datasources.containsKey(lang) || inactiv.containsKey(lang);
            }
