/**
 * $Header: /usr/cvs/jlani2/src/de/uni_leipzig/asv/toolbox/jLanI/kernel/LanIKernel.java,v 1.3 2009-04-09 10:07:58 steresniak Exp $
 * 
 ...
 */
public class LanIKernel implements LanIKernelInterface {

    public static final String version = "$Revision: 1.3 $";

    public static String propertyFile = "lanikernel";
    ...
    private LanIKernel() throws DataSourceException {
        ...
        if ( this.prefs.getProperty( "KernelLogMode" ) != null ) {
            try {
                ...
                this.log
                        .log( "LanIKernel: setting KernelLogMode to default: "
                                + Log.DEFAULT_LOGMODE );
            }
        } else {
        ...
        if ( this.prefs.getProperty( "BlacklistFile" ) == null
            ...
            this.log.log( "LanIKernel: no blacklist-file given, that's not a real problem" );
     ...
     * @throws DataSourceException
     */
    public synchronized static LanIKernel getInstance() throws DataSourceException {
    ...
    public void reset() {
        ...
        this.prefs = null;
        this.log
                .debug( "LanIKernel: removing reference to datasource manager" );
    ...
    public boolean dumpDatasource( String lang, String filename ) {
        if ( this.datasourcemanager.hasLanguage( lang ) ) {
            return this.datasourcemanager
                    .serializeDatasource( lang, filename );
        }
     ...
     * @throws DataSourceException
     * 
     * @see LanIKernelInterface.kernel#evaluate(jLanI.Request)
     */
    public Response evaluate( Request request ) throws RequestException,
            DataSourceException {

        this.log.debug( "LanIKernel: evaluate() called with request '"
                + request + "'" );
        ...
        // clean the sentences, e.g. remove special chars like !"ÃÂÃÂ§$%&/() ...
        ...
        for ( int i = 0; i < splittedsentence.length; i++ ) {
            ...
            for ( Iterator iter = request.getLanguages().iterator(); iter
				...
				double tmpValue = tempdouble.doubleValue()*((Double)temp.get(lang)).doubleValue();
        ...
        this.log.debug( new StringBuffer(
                "LanIKernel: returning response object: '" )
        ...
        if ( !reduced ) {
            ...
            for ( Iterator iter = response.getSeenWords().keySet().iterator(); iter
                ...
                double coverageVal = ((double)(((LinkedList)response
                        ...
                        / sentencelength;
                int countVal=((LinkedList)response.getSeenWords().get( lang )).size();
                coverage.put( lang, (coverageVal == Double.NaN) ? new Double(
                        0.0 ) : new Double( coverageVal ) );
    ...
    /**
     * 
     * @see LanIKernelInterface.kernel#getAvailableDatasources()
     */
...
//                if ( tokenfile.canRead() ) {
                    this.datasourcemanager.addDatasource( tempstring,
                            wordlistdir
                                    + System.getProperty( "file.separator" )
                                    + tempstring + ".txt", wordlistdir
                                    + System.getProperty( "file.separator" )
                                    + tempstring + ".tok" );
//                } else {
//                    this.log
...
//                }
    ...
    public void test() throws RequestException, DataSourceException {
        ...
        req.setReduce( false );
        req
                .setSentence( "my pony is over the ocean, my bonny is over the see" );
        ...
        req
                .setSentence( "ÃÂÃÂ° und last but not least, bin ich ein _kurzer_ deutscher Satz (hubergel)!" );
        System.out.println( "\"" + req.getSentence() + "\"" );
